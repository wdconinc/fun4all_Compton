...
// ComptonTruthDetector::ConstructMe() method
// but the convention is as mentioned cm and deg
//____________________________________________________________________________..
//

/*
Subsystem -> takes in list of detector names, passed it on to stepping action and event action
simple detector planes -> create
event action -> init tree of vector<X>; fill tree; write tree
stepping action -> fill G4Hits
*/
...
ComptonTruthSubsystem::ComptonTruthSubsystem(const std::string &name)
...
//_______________________________________________________________________
int ComptonTruthSubsystem::InitRunSubsystem(PHCompositeNode *topNode)
{
  ...
  // create detector
  m_Detector = new ComptonTruthDetector(this, topNode, GetParams(), Name());
  m_Detector->OverlapCheck(CheckOverlap());
  m_Detector->Verbosity(Verbosity());
  // create stepping action if detector is active
  if (GetParams()->get_int_param("active"))
  {
    m_SteppingAction = new ComptonTruthSteppingAction(m_Detector, GetParams(),m_TrackLevel);
    m_SteppingAction->Verbosity(Verbosity());
  }
  return 0;
}
//_______________________________________________________________________
int ComptonTruthSubsystem::process_event(PHCompositeNode *topNode)
{
...
//_______________________________________________________________________
void ComptonTruthSubsystem::Print(const string &what) const
{
  if (m_Detector)
  {
...
//_______________________________________________________________________
PHG4Detector *ComptonTruthSubsystem::GetDetector(void) const
{
  return m_Detector;
}

//_______________________________________________________________________
void ComptonTruthSubsystem::SetDefaultParameters()
{
  // sizes are in cm
  // angles are in deg

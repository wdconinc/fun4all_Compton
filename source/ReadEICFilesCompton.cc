...
ReadEICFilesCompton::ReadEICFilesCompton(const string &name):
  SubsysReco(name),
  filename(""),
  Tin(nullptr),
  nEntries(0),
...
ReadEICFilesCompton::~ReadEICFilesCompton()
{
  delete Tin;
}
...
bool
ReadEICFilesCompton::OpenInputFile(const string &name)
{
  filename = name;
...
void ReadEICFilesCompton::GetTree()
{
  /* Print the actual class of the event branch record,
     i.e. erhic::EventMilou or other */
  cout << "ReadEICFilesCompton: Input Branch Event Class = "
...
int
ReadEICFilesCompton::Init(PHCompositeNode *topNode)
{
  /* Create node tree */
...
int
ReadEICFilesCompton::process_event(PHCompositeNode *topNode)
{
  ...
  if ( hepmc_particles.size() != origin_index.size() )
    {
      cout << "ReadEICFilesCompton::process_event - Lengths of HepMC particles and Origin index vectors do not match!" << endl;
  ...
  for ( unsigned p = 2; p < hepmc_particles.size(); p++ )
    {
      if ( ! hepmc_particles.at(p)->production_vertex() )
        {
          cout << "ReadEICFilesCompton::process_event - Missing production vertex for one or more non-beam particles!" << endl;
          delete evt;
          return Fun4AllReturnCodes::ABORTRUN;
        }
    }
  ...
  if (!success) {
    cout << "ReadEICFilesCompton::process_event - Failed to add event to HepMC record!" << endl;
    return Fun4AllReturnCodes::ABORTRUN;
  }
...
int ReadEICFilesCompton::CreateNodeTree(PHCompositeNode *topNode) {

...
//____________________________________________________________________________..
ComptonTruthSteppingAction::ComptonTruthSteppingAction(ComptonTruthDetector *detector, const PHParameters *parameters, int trackLevel)
  ...
  , m_EdepSum(0)
  , m_EionSum(0)
  , m_TrackLevel(trackLevel)
{
}

//____________________________________________________________________________..
ComptonTruthSteppingAction::~ComptonTruthSteppingAction()
{
...
bool ComptonTruthSteppingAction::UserSteppingAction(const G4Step *aStep,bool was_used)
{
  ...
  if( !(tid == 1 || tid == 2) && m_TrackLevel == 1 ) return false;
  ...
  if(Verbosity()>3){
    ...
    cout<<"\tPDG encoding "<< aTrack->GetParticleDefinition()->GetPDGEncoding() <<" "<< aTrack->GetParticleDefinition()->GetParticleName()<<endl;
    cout<<"\ttrack ID "<<aTrack->GetTrackID()<<endl;
    if(m_SaveVolPre && m_SaveVolPost)
  ...
  case fUndefined:
    ...
    if(Verbosity()>3){
      cout<<"\trecording track ID "<<aTrack->GetTrackID()<<endl;
    }
  ...
  if(Verbosity()>3){
    cout<<__PRETTY_FUNCTION__<<" : "<<__LINE__<<endl;
    cout<<"\t mom "<<prePoint->GetMomentum()<<"\tpos\t"<<prePoint->GetPosition()<<endl;
  }
...
//____________________________________________________________________________..
void ComptonTruthSteppingAction::SetInterfacePointers(PHCompositeNode *topNode)
{
  ...
  if (!m_HitContainer)
    {
      std::cout << "ComptonTruthSteppingAction::SetTopNode - unable to find "
